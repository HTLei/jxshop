<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:69:"D:\item\www.shop.com\public/../application/admin\view\user\index.html";i:1535876833;}*/ ?>
        ...
        <table class="tablelist">
            ...
            <?php if(is_array($users) || $users instanceof \think\Collection || $users instanceof \think\Paginator): if( count($users)==0 ) : echo "" ;else: foreach($users as $key=>$user): ?>
                <tr>
                    ...
                    <td><?php echo $user['create_time']; ?></td>
                    <td><a href="<?php echo url('/admin/user/upd',['user_id'=>$user['user_id']]); ?>"  target="rightFrame" class="tablelink">编辑</a> <a href="<?php echo url('/admin/user/del',['user_id'=>$user['user_id']]); ?>"  target="rightFrame" onclick="return confirm('确定要删除吗？');" class="tablelink"> 删除</a></td>
                </tr>
